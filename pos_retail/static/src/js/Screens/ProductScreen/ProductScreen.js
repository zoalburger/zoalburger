        ...
        class extends ProductScreen {
            ...
            async infoCartSummary() {
                ...
                for (let categ in values) {
                    lists.push({
                        id: categ,
                        label: this.env.pos.format_currency_no_symbol(values[categ]) + ' - ' + categ,
                        item: categ
                    })
                }
                this.showPopup('SelectionPopup', {
                    title: this.env._t('Cart Summary Items [Quantity - Category]'),
                    list: lists,
                    ...
                    if (this.env.pos.config.validate_minimum_price && this.env.pos.user.role != 'manager') {
                        for (let i = 0; i < selectedOrder.orderlines.models.length; i++) {
                            ...
                            if (warningMessage) {
                                ...
                                const waitingApproveId = await selectedOrder._requestApproveAction(action, description, action_strId, type)
                            ...
                            if (line.waitingApproveId) {
                                ...
                                if (!requestStatus) {
                                    this.showPopup('ErrorPopup', {
                                        title: this.env._t('Warning'),
                                        body: this.env._t("You need Request Manager approve Action ID ") + line.uid
                                    })
                                    return false
                                }
                            }
            ...
            async _clickProduct(event) {
                ...
                if (this.env.pos.config.fullfill_lots && ['serial', 'lot'].includes(event.detail.tracking)) {
                    ...
                    let packLotLinesToEdit = await this.rpc({
                        ...
                        method: 'search_read',
                        domain: [['product_id', '=', event.detail.id], ['product_qty', '>', 0]],
                        fields: []
                    }).then(function (value) {
                        return value
                    }, function (error) {
