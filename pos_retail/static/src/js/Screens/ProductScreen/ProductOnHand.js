                ...
                for (let location_id in stock_datas) {
                    ...
                    if (location) {
                        ...
                        if (this.env.pos.config.notification_out_of_stock > 0 && this.state.qty_available <= qty_available) {
                            ...
                            this.env.pos.alert_message({
                                title: this.props.product.display_name + this.env._t(' have warning stock'),
                                body: this.props.product.display_name + this.env._t(' only available for sale: ') + this.env.pos.format_currency_no_symbol(this.state.qty_available)
                            })
                        }
                        break
                    }
