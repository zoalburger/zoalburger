odoo.define('pos_retail.OrderCartAction', function (require) {
    ...
    class OrderCartAction extends PosComponent {
        ...
        get addedClasses() {
            ...
            const changes = this.env.pos.get_order().hasChangesToPrint();
            const skipped = changes ? false : this.env.pos.get_order().hasSkippedChanges();
            return {
                highlight: changes,
                altlight: skipped,
            };
        ...
        get payButtonClasses() {
            ...
            if (this.env.pos.config.module_pos_restaurant && this.env.pos.config.is_table_management && this.env.pos.config.floor_ids && this.env.pos.config.floor_ids.length > 0) {
                restaurant = true
            }
            return {
                highlight: highlight,
