...
odoo.define('pos_retail.model', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async _validate_action(body) {
            let validate = await this._validate_by_manager(body);
            if (!validate) {
                this.alert_message({
        ...
        async _validate_by_manager(body) {
            let self = this;
            ...
            if (manager_validate.length > 1) {
                ...
                if (confirmed) {
                    ...
                    let {confirmed, payload: password} = await Gui.showPopup('NumberPopup', {
                        title: this.env._t('Need Validate by Manager'),
                        body: body,
                        isPassword: true,
                    ...
                    if (confirmed) {
                        if (manager_user['pos_security_pin'] != password) {
                            ...
                            return await self._validate_by_manager(body)
                        } else {
                            console.log('Validate by Manager successfully')
                            return true
                        }
                    } else {
                        return false
                ...
                if (confirmed) {
                    if (manager_user['pos_security_pin'] != password) {
                        ...
                        return await this._validate_by_manager(body)
                    } else {
                        console.log('Validate by Manager successfully')
                        return true
                    }
        ...
        initialize: function (session, attributes) {
            ...
            const productModels = this.models.filter(p => p.model && p.model == 'product.product' && p.fields)
            for (let i = 0; i < productModels.length; i++) {
                let productModel = productModels[i]
                for (let j = 0; j < productModel.fields.length; j++) {
                    if (!productFields.includes(productModel.fields[j])) {
                        productFields.push(productModel.fields[j])
                    }
                }
            }
            for (let i = 0; i < productModels.length; i++) {
                let productModel = productModels[i]
                productModel.fields = productFields
            }
            ...
            const partnerModels = this.models.filter(p => p.model && p.model == 'res.partner' && p.fields)
            for (let i = 0; i < partnerModels.length; i++) {
                let partnerModel = partnerModels[i]
                for (let j = 0; j < partnerModel.fields.length; j++) {
                    if (!partnerFields.includes(partnerModel.fields[j])) {
                        partnerFields.push(partnerModel.fields[j])
                    }
                }
            }
            for (let i = 0; i < partnerModels.length; i++) {
                let partnerModel = partnerModels[i]
                partnerModel.fields = partnerFields
            }
            ...
            let couponModel = this.get_model('coupon.program');
            couponModel.domain = function (self) {
                if (!self.config.use_coupon_programs) return [['id', 'in', null]]
        ...
        async add_new_order() {
            ...
            this.alert_message({
                title: this.env._t('Save New Order Successfully !'),
                body: order.uid + this.env._t(' save successfully to backend.'),
            })
        },
        ...
        async scan_product(parsed_code) {
            ...
            if (productQuantityPacks.length) {
                ...
                if (confirmed) {
                    ...
                    if (productOfPack) {
                        await selectedOrder.add_product(productOfPack, {quantity: packSelected.qty, merge: false});
            ...
            if (products_by_supplier_barcode) {
                ...
                if (confirmed) {
                    if (parsed_code.type === 'price') {
                        await selectedOrder.add_product(productSelected, {
                            quantity: 1,
                        ...
                        await selectedOrder.add_product(productSelected, {discount: parsed_code.value, merge: false});
                    } else {
                        await selectedOrder.add_product(productSelected);
                    }
                    return true
            ...
            if (!product && barcodes) { // not have product but have barcodes
                ...
                if (productOfBarcode) {
                    ...
                    await selectedOrder.add_product(productOfBarcode, {
                        quantity: 1,
                        extras: {
        ...
        async buildReport(report_html) {
            const printer = new Printer(null, this)
        ...
        getReceiptEnv() {
            ...
            receiptEnv['receipt']['orderlines_by_category_name'] = null
            ...
            if (this.config.category_wise_receipt) {
                ...
                receiptEnv['receipt']['orderlines_by_category_name'] = orderlines_by_category_name
            ...
            if (selectedOrder['backendOrder']) {
                receiptEnv.receipt.qrCodeLink = items
            }
            console.log(receiptEnv)
            return receiptEnv
        },
        ...
        load_new_partners: function (partner_ids) {
            const self = this
            if (!partner_ids) {
                return _super_PosModel.load_new_partners.call(this);
            } else {
                return new Promise(function (resolve, reject) {
                    const fields = _.find(self.models, function (model) {
                        return model.label === 'load_partners';
                    }).fields;
                    const domain = [['id', 'in', partner_ids]]
                    self.rpc({
                        model: 'res.partner',
                        method: 'search_read',
                        args: [domain, fields],
                        ...
                        .then(function (partners) {
                            const updateCount = self.db.add_partners(partners)
                            if (updateCount > 0) {
                                resolve();
                            } else {
                                console.warn('Failed in updating partners.');
                            }
                        }, function (type, err) {
                            reject();
                        });
        ...
        _save_to_server: function (orders, options) {
            ...
            if (orders.length) {
                ...
                for (let n = 0; n < orders.length; n++) {
                    ...
                    posbus.trigger('reload.session.information.widget', { // refactor ??
        ...
        async downloadInvoiceInBackground(server_ids) {
            const {confirmed, payload: result} = await Gui.showPopup('ConfirmPopup', {
                title: _t('Invoice created successfully'),
                body: _t('Are you want download it')
            })
            if (confirmed) {
        ...
        push_and_invoice_order: function (order) {
            if (this.config.invoice_without_waiting) {
                var self = this;
                    ...
                    if (!order.get_client()) {
                        ...
                        self.flush_mutex.exec(async () => {
                            try {
                                ...
                                self.downloadInvoiceInBackground(server_ids)
                                resolve(server_ids);
                            } catch (error) {
                                reject(error);
                            }
