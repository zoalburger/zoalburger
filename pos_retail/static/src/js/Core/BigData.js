odoo.define('pos_retail.big_data', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        initialize: async function (session, attributes) {
            ...
            this.product_onhand_by_location_id = {}
        ...
        saveProductOnHandByLocation(productsOnHandByLocation) {
            for (let location_id in productsOnHandByLocation) {
                if (!this.product_onhand_by_location_id[parseInt(location_id)]) {
                    this.product_onhand_by_location_id[parseInt(location_id)] = {}
                }
                for (let product_id in productsOnHandByLocation[location_id]) {
                    if (parseInt(product_id) != 0) {
                        this.product_onhand_by_location_id[location_id][parseInt(product_id)] = productsOnHandByLocation[location_id][parseInt(product_id)]
                    }
                }
            }
            console.log('Save stock on hand each product each location')
            console.log(this.product_onhand_by_location_id)
        },


        async getStockDatasByLocationIds(product_ids = [], location_ids = []) {
            if (product_ids.length == 0 || product_ids.length > 1) {
                let productsOnHandByLocation = {}
                try {
                    productsOnHandByLocation = await this.rpc({
                        model: 'stock.location',
                        method: 'getStockDatasByLocationIds',
                        args: [[], product_ids, location_ids],
                        context: {}
                    })
                    if (productsOnHandByLocation) {
                        this.saveProductOnHandByLocation(productsOnHandByLocation)
                    }
                ...
                return this.product_onhand_by_location_id
            }
        },

        // todo: we need redeem stock after sold out products
        _save_to_server: function (orders, options) {
            if (orders.length && !options['draft']) {
                let location = this.get_source_stock_location()
                if (this.config.display_onhand && this.product_onhand_by_location_id[location.id])
                    for (let i = 0; i < orders.length; i++) {
                        let lines = orders[i]['data']['lines']
                        for (let j = 0; j < lines.length; j++) {
                            let line = lines[j][2]
                            let product_id = line['product_id']
                            let product = this.db.get_product_by_id(product_id)
                            if (product.type == 'product' && this.product_onhand_by_location_id[location.id][product_id]) {
                                this.product_onhand_by_location_id[location.id][product_id] = this.product_onhand_by_location_id[location.id][product_id] - line['qty']
                                console.log(product.display_name + ' just redeem ' + line['qty'] + ' from location: ' + location.name)
                            }
                        }
                    }
            }
            const res = _super_PosModel._save_to_server.call(this, orders, options)
            return res
        },
