...
odoo.define('pos_retail.order', function (require) {
    ...
    models.Order = models.Order.extend({
        initialize: function (attributes, options) {
            ...
            if (this.pos.config.promotion_auto && this.pos.config.promotion_ids.length > 0) {
                this.on(
                    'update-promotion',
                    () => {
                        if (!this.pos.config.promotion_ids) return;
            ...
            this.on('remove-promotion', this.remove_all_promotion_line, this);
            this.on(
                'automatic-wallet-payment',
                () => {
                    const self = this
                    let walletMethod = this.pos.payment_methods.find((p) => p.journal && p.pos_method_type == 'wallet')
                    if (walletMethod) {
                        let paymentLines = this.paymentlines.models
                    ...
                    if (!this.get_client() || !walletMethod || (this.get_client() && this.get_client().wallet <= 0)) return false
                    let amountDue = this.get_total_with_tax() + this.get_rounding_applied()
                    let startingValue = 0;
                    let clientWallet = this.get_client().wallet
                    ...
                    if (amountDue > 0) {
                        this.add_paymentline(walletMethod);
                        let paymentline = this.selected_paymentline;
                        paymentline.set_amount(startingValue);
            ...
            this.on(
                ...
                async () => {
                    const self = this
                    let creditMethod = this.pos.payment_methods.find((p) => p.journal && p.pos_method_type == 'credit')
                    if (creditMethod) {
                        let paymentLines = this.paymentlines.models
                        paymentLines.forEach(function (p) {
                    ...
                    if (!this.get_client() || !creditMethod || (this.get_client() && this.get_client().balance <= 0)) return false
                    let amountDue = this.get_total_with_tax() + this.get_rounding_applied()
                    let startingValue = 0;
                    let clientCreditBalance = this.get_client().balance
                    if (clientCreditBalance >= amountDue) {
                        startingValue = amountDue
                    } else {
                        startingValue = clientCreditBalance
                    }
                    if (amountDue > 0) {
                        this.add_paymentline(creditMethod);
                        let paymentline = this.selected_paymentline;
                        paymentline.set_amount(startingValue);
                        this.trigger('change', this);
                    }
        ...
        async validateRequestApproveOrNot(line_uid) {
            const requestStatus = await rpc.query({
                model: 'pos.approve.action',
                method: 'check_from_ui',
                args: [[], line_uid],
                context: {}
            })
        ...
        async setBundlePackItems() {
            ...
            if (selectedLine) {
                ...
                return this.pos.alert_message({
                    title: _t('Error'),
                    body: _t('Have not any selected line in cart, Please selected 1 Line')
                })
            }
        },
        async suggestItems() {
            ...
            if (confirmed) {
                ...
                for (let index in selectedCrossItems) {
                    ...
                    if (product) {
                        ...
                        await selectedOrder.add_product(product, {
        ...
        set_client: async function (client) {
            ...
            if (client && this.pos.config.notification_last_days_purchase && !this.pos.the_first_load) {
                this.pos.rpc({
                    model: 'pos.config',
                    method: 'checkCustomerLastPurchase',
                    args: [[], client.id],
                    context: {}
                }, {
                    shadow: true,
                    ...
                    if (days > 0) {
                        if (days == 1) {
                            self.pos.alert_message({
                                title: client.name,
                                body: _t('just purchase today')
                            })
                        } else {
                            self.pos.alert_message({
                                title: client.name,
                                body: days + _t(' days not purchase')
                            })
                        }
                    } else {
                        self.pos.alert_message({
                            title: client.name,
                            body: _t('First time purchase')
                        })
                    }
                }, function (err) {
                    return self.pos.query_backend_fail(err)
            ...
            this.trigger('automatic-wallet-payment')
            this.trigger('automatic-credit-payment')
            return res
        },
        async add_shipping_cost(service, product, is_shipping_cost) {
        ...
        async _appliedDiscountValue(lines, discount, amount_withtax) {
            let discountProduct = this.pos.db.product_by_id[this.pos.config.discount_global_id[0]];
            if (discountProduct) {
                await this.add_product(discountProduct, {quantity: -1, merge: false});
                ...
                for (let item_index in reward['gift_product_ids']) {
                    ...
                    if (product_gift) {
                        ...
                        if (client['pos_loyalty_point'] > (point_will_redeem + redeem_point_used)) {
                            applied = true;
                            await this.add_product(product_gift, {
                                price: 0,
        ...
        compute_buy_x_get_another_free: function (promotion) {
            ...
            for (let index_id in promotion.product_ids) {
                let product_id = promotion.product_ids[index_id];
                if (total_quantity[product_id] && total_quantity[product_id] >= minimum_items) {
                    let qty_free = parseInt(total_quantity[product_id] / minimum_items)
                    let product = this.pos.db.product_by_id[product_id];
                    if (!product) {
                        return this.pos.alert_message({
                            title: _t('Error'),
    ...
    models.Orderline = models.Orderline.extend({
        initialize: function (attributes, options) {
            ...
            if (!options.json) {
                ...
                this.discount_has_valid = false
                this.price_has_valid = false
            }
            return res;
        },
        ...
        init_from_JSON: function (json) {
            ...
            if (json.discount_has_valid) {
                this.discount_has_valid = json.discount_has_valid
            }
            if (json.price_has_valid) {
                this.price_has_valid = json.price_has_valid
            }
            if (json.warning_message) {
                this.warning_message = json.warning_message
            }
            if (json.waitingApproveId) {
                this.waitingApproveId = json.waitingApproveId
            }
        },
        ...
        export_as_JSON: function () {
            ...
            if (this.discount_has_valid) {
                json.discount_has_valid = this.discount_has_valid
            }
            if (this.price_has_valid) {
                json.price_has_valid = this.price_has_valid
            }
            if (this.warning_message) {
                json.warning_message = this.warning_message
            }
            if (this.waitingApproveId) {
                json.waitingApproveId = this.waitingApproveId
            }
            return json;
        },
        ...
        clone: function () {
            ...
            orderline.discount_has_valid = false;
            orderline.price_has_valid = false;
            return orderline;
        },
        ...
        verifyMinimumPriceOfProduct(lastPrice, priceApplied) {
            if (priceApplied > 0 && ((this.product.minimum_price > 0 && this.product.minimum_price > priceApplied && priceApplied < this.product.get_price())) || (priceApplied < lastPrice)) {
        ...
        set_unit_price: async function (price) {
            ...
            if (!this.is_return && this.pos.config.validate_minimum_price) {
                const minimumPrice = this.verifyMinimumPriceOfProduct(this.last_price, this.after_price)
                if (minimumPrice) {
                    this.pos.alert_message({
                        title: _t('Warning'),
                        body: minimumPrice
            ...
            if (this.pos.config.validate_price_change && currentPrice != this.product.get_price()) {
                ...
                const waitingApproveId = await this.order._requestApproveAction(action, description, action_strId, type)
        ...
        async set_discount(discount) {
            ...
            if (!this.pos.the_first_load && this.pos.config.discount_limit && discount > this.pos.config.discount_limit_amount) {
                let validate = await this.pos._validate_action(_t('Need set Discount: ') + discount + ' % but POS Config set limited can set Discount ' + this.pos.config.discount_limit_amount + ' %.');
                if (!validate) {
                    return this.pos.alert_message({
                        title: _t('Error'),
