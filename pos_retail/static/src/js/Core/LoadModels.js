...
odoo.define('pos_retail.load_models', function (require) {
    ...
    exports.posSyncBackend = Backbone.Model.extend({
        ...
        start: function () {
            ...
            if (this.pos.config.sync_products_realtime) {
                this._automaticUpdate()
            }
        },
        ...
        async on_notification(notifications) {
            if (notifications && notifications[0] && notifications[0][1]) {
                ...
                if (type == 'pos.sync.pricelists') {
                    console.warn('!!! event on_notification()')
                    console.log(type)
                    console.log(payload)
                    this.reload_pricelists()
                }
                if (type == 'pos.modifiers.background') {
                    console.warn('!!! event on_notification()')
                    console.log(type)
                    console.log(payload)
                    this.force_update_ui(payload)
                }
            }
        },
            ...
            if (pricelists_model) {
                ...
                return first_load.then(function () {
                    ...
                    return second_load.then(function () {
                        ...
                        if (order && !order.finalized && pricelist) {
        ...
        async _automaticUpdate() {
            this.pos._syncPartners()
            this.pos._syncProducts()
            setTimeout(_.bind(this._automaticUpdate, this), 15000);
        },
    });
    ...
    models.load_models([
        ...
        },
        {
            ...
            condition: function (self) {
                return self.stock_location_ids && self.stock_location_ids.length > 0 && self.config.display_onhand;
            },
            loaded: async function (self) {
                await self.getStockDatasByLocationIds([], self.stock_location_ids)
            },
        },
        {
    ...
    let extend_models = [
        ...
        },
        {
            ...
            fields: ['name', 'code', 'pos_method_type', 'profit_account_id', 'loss_account_id', 'currency_id', 'decimal_rounding'],
            domain: function (self) {
                ...
                if (self.payment_methods) {
                    for (let i = 0; i < self.payment_methods.length; i++) {
                        ...
                        if (payment_method.journal_id) {
                            payment_method.journal = self.journal_by_id[payment_method.journal_id[0]];
