...
class PosConfig(models.Model):
    ...
    def updateCache(self):
        return self.env['pos.call.log'].refresh_logs(True)
    ...
    client_search_field_ids = fields.Many2many(
        'ir.model.fields',
        'pos_config_search_client_field_rel',
        'config_id',
        'field_id',
        domain=[
            ('model', '=', 'res.partner'),
            ('ttype', 'not in', ['binary', 'many2one', 'one2many', 'many2many', 'boolean'])
        ],
        string='Can find customer with field',
        help='Customer can find via field you add here'
    )
    product_search_field_ids = fields.Many2many(
        'ir.model.fields',
        'pos_config_search_product_field_rel',
        'config_id',
        'field_id',
        domain=[
            ('model', '=', 'product.product'),
        ...
        string='Can find product with field',
        help='Product can find via field you add here'
    )
    ...
    load_orders_limit = fields.Integer(
        default=100,
        string='Limit Orders Loading To POS',
        help='Another Order not load to POS, you can type to search box with name, ean13, note .... of Order'
    )
    ...
    sync_multi_session_manual_stop = fields.Boolean('Sync Can stop by User')
    sync_multi_session_alert_remove_order = fields.Boolean('Alert when another Sessions Remove Orders')
    ...
    sync_multi_session_offline = fields.Boolean(
        'Sync Between Session with Local Network (Partnership only)',
        default=0,
    ...
    sync_multi_session_offline_iot_ids = fields.Many2many(
        ...
        'iot_box_id',
        string='IoT Boxes for Sync (Partnership only)',
    ...
    validate_minimum_price = fields.Boolean('Validate Product has Sale Price smaller than Minimum Price')
    ...
    load_booked_orders_limit = fields.Integer(
        default=100,
        string='Limit Sale Orders Loading To POS',
        help='Another Order not load to POS, you can type to search box with name, ean13, note .... of Order'
    )
    ...
    display_minimum_price = fields.Boolean('Display Minimum Price', default=0)
    ...
    load_invoices_limit = fields.Integer(
        default=100,
        string='Limit Invoices Loading To POS',
        help='Another Invoice not load to POS, you can type to search box with name, number .... of Order'
    )
    ...
    invoice_without_waiting = fields.Boolean(
        default=1,
        string='Order to Invoice without Waiting',
        help='Normally Odoo original, when order to invoice \n'
             'Cashier need waiting times for submit order and download a invoice \n'
             'Feature can help cashier by pass waiting time for download invoice \n'
             'And when order to invoice finished, pos will ask cashier need download invoice or No\n'
             'If cashier say no, them keep sale products'
    )
    ...
    qrcode_receipt = fields.Boolean(
        'Display QrCode',
        default=1,
        help='If checked, on Receipt Order will display Qrcode \n'
             'When customer back and need return products \n'
             'Cashier can open Camera of POS Device \n'
             'And scan Qrcode for return the products'
    )
    ...
    receipt_fix = fields.Boolean('Receipt Fixed')
    receipt_fix_width = fields.Integer('Receipt Width (mm)', default=0)
    receipt_fix_margin = fields.Integer('Receipt Margin (mm)', default=0)
    receipt_fix_margin_left = fields.Integer('Receipt Margin Left (mm)', default=0)
    receipt_fix_margin_right = fields.Integer('Receipt Margin Right (mm)', default=0)
    receipt_fix_font_size = fields.Integer(
        'Receipt Font Size (px)',
        default=13,
        help='Set range 13px to 16px'
    )
    ...
    report_payment_summary = fields.Boolean(string="Payments Summary", default=0)
    ...
    promotion_auto = fields.Boolean('Promotions Auto Apply')
    ...
    replace_paid_button_to_assign = fields.Boolean(
        'Replace Paid Button to Assign Button',
        help='If checked, button Paid of POS screen will change to Assign button'
    )
    assign_orders_to_config_ids = fields.Many2many(
        ...
        'from_config_id',
        'assign_config_id',
        string='POS Received Assign Orders'
    )
    ...
    customer_facing_screen_auto = fields.Boolean(
        'Auto open Customer Facing Screen',
        help='When Started POS, Customer Facing auto Open',
        default=0
    )
    ...
    gift_card_screen = fields.Boolean(string="Gift Card Screen")
    default_show_all_buttons_feature = fields.Boolean(
        'Default Show all Buttons Feature',
        default=1
    )
    ...
    header_background_color = fields.Char('Header Background App', default='#3e373c')
    cart_box_style = fields.Selection([
        ('left', 'Left of POS Screen'),
        ('right', 'Right of POS Screen')
    ], default='left', required=1, string='Cart Items Box Position')
    ...
    credit = fields.Boolean('Provide Credit to Customer')
    credit_product_id = fields.Many2one(
        'product.product',
        domain=[('is_credit', '=', True), ('available_in_pos', '=', True)],
        help='Please choice product Credit is checked and Available In Pos is checked'
    )

    notification_payment_time = fields.Integer(
        'Notification Payment Time (minutes)',
        default=0,
        help='If set bigger than 0, each minute number here will notification \n'
             'List out all customer not yet payment full'
    )
    notification_out_of_stock = fields.Integer(
        'Notification when Products Out of Stock',
        default=0,
        help='Set minimum quantity of product available stock here \n'
             'If product quantity smaller than this number \n'
             'POS automatic send notification to Notification menu'
    )
    notification_dead_stock = fields.Integer(
        'Notification Dead Stock',
        default=0,
        help='Each minutes will notification'
    )
    notification_dead_stock_days = fields.Integer(
        'Notification Dead Days',
        default=7,
        help='Days products not sell'
    )
    notification_last_days_purchase = fields.Boolean(
        'Notification last days Purchase',
        help='When cashier set customer, will have one popup \n'
             'Customers details notified to users as they are not purchasing for a while or x days'
    )

    def checkCustomerLastPurchase(self, client_id):
        order = self.env['pos.order'].search([
            ('partner_id', '=', client_id)
        ], limit=1, order='create_date DESC')
        if order:
            diffDays = (fields.Datetime.now() - order.create_date).days
            if diffDays == 0:
                diffDays = 1
            return diffDays
        else:
    ...
    def startingNotificationPayment(self, config_id):
        return self.env['pos.order'].search_read([
            ('config_id', '=', config_id),
            ('state', 'in', ['draft', 'partial_payment']),
            ('amount_total', '>', 0)
        ], [
            'name',
            'ean13',
    ...
    def startingNotificationDeadStock(self, config_id):
        config = self.browse(config_id)
        checkDate = fields.Datetime.now() - timedelta(days=config.notification_dead_stock_days)
        ...
        productsSold = []
        for l in lines:
            if l.product_id.id not in productsSold:
                productsSold.append(l.product_id.id)
        return self.env['product.product'].search_read([
            ('id', 'not in', productsSold),
    ...
    def reinstall_database(self):
        ...
        for session in sessions_opened:
            self.env['bus.bus']._sendone(
                session.user_id.partner_id, 'pos.remote_sessions', {
                    'remove_cache': True,
            ...
            if sessions:
                for session in sessions:
                    if session.cash_control and abs(
                        ...
                        session.close_session_without_popup()
                    vals = {
                    ...
                    self.env['bus.bus']._sendone(
                        session.user_id.partner_id, 'pos.remote_sessions', vals)
            else:
    ...
    def forceChangeUI(self):
        for config in self:
            ...
            if sessions:
                ...
                for s in sessions:
                    self.env['bus.bus']._sendone(
                        s.user_id.partner_id, 'pos.modifiers.background', config)
        return True
