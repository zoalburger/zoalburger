...
class PosSession(models.Model):
    ...
    def close_session_without_popup(self, balancing_account=False, amount_to_balance=0, bank_payment_method_diffs={}):
        result = None
        for session in self:
            result = session.action_pos_session_close(balancing_account, amount_to_balance, bank_payment_method_diffs)
            if result != True and result.get('res_id', None):
                wiz = self.env['pos.close.session.wizard'].browse(result.get('res_id'))
                session.action_pos_session_closing_control(
                    wiz.account_id, wiz.amount_to_balance
                )
            if result != True and result.get('res_model', None) == 'closing.balance.confirm.wizard':
                confirmClose = session._validate_session(balancing_account, amount_to_balance,
                                                         bank_payment_method_diffs)
            _logger.info('Closed Session %s direct POS' % session.name)
        return result
    ...
    def action_pos_session_closing_control(self, balancing_account=False, amount_to_balance=0,
        ...
        for session in self:
            ...
            if len(orders) > 0:
                _logger.warning('orders not full fill payment: %s' % orders)
                for order in orders:
    ...
    def _get_backup_session(self, order):
        ...
        new_session = self.create({
            'config_id': closed_session.config_id.id,
            'name': _('(SESSION BACKUP FOR %(session)s, save Orders not full full payments)') % {
                'session': closed_session.name},
            'rescue': True,
            'backup_session': True,
        })
    ...
    def get_session_online(self):
        return {
            'session_online': len(self.sudo().search([('state', '=', 'opened')])),
            'module_installed': True if self.env['ir.module.module'].sudo().search([('name', '=', 'pos_retail')],
                                                                                   limit=1) else False
        }
